...
var sequelize = new Sequelize('catalog', 'ileana', 'password1', {
    dialect:'mysql',
    host:'localhost'
})
...
var Order = sequelize.define('orders', {
    ...
    specials:Sequelize.STRING
})

Products.belongsTo(Categories, {foreignKey: 'category_id', targetKey: 'id'})
Order.belongsTo(Products, {foreignKey: 'product_id', targetKey: 'id'})
...
app.delete('/orders/:id', function(request, response) {
    Order.findById(request.params.id).then(function(orders) {
        if(orders) {
            orders.destroy().then(function(){
                response.status(204).send()
            })
        } else {
            response.status(404).send('Not found')
...
app.post('/orders', function(request, response) {
    Order.create(request.body).then(function(orders) {
